...
def test_desktop_mode_rejects_non_local_sandbox_on_reload(tmp_path, monkeypatch):
    config_path = tmp_path / "config.yaml"
    extensions_path = tmp_path / "extensions_config.json"
    _write_extensions_config(extensions_path)
    _write_config(config_path, model_name="first-model", supports_thinking=False)
    ...
    try:
        initial = get_app_config()
        assert initial.sandbox.use == "deerflow.sandbox.local:LocalSandboxProvider"
        ...
        with pytest.raises(ValueError, match="Desktop mode only supports LocalSandboxProvider"):
            get_app_config()
    finally:
        reset_app_config()
